...
public:
    PointCloudTools(){
        ...
        // Read exiting point cloud models
        // readExistingPointCloudTemplates("/home/clarence/ros_ws/semantic_dsp_ws/src/Semantic_DSP_Map/data/VirtualKitti2/template");
    ...
    /// @return 
    int generateLabeledPointCloud(const cv::Mat &depth_value_mat, const std::vector<MaskKpts> &ins_seg_result,
                                  std::vector<std::vector<LabeledPoint>> &cloud, std::unordered_map<uint16_t, std::vector<Eigen::Vector3d>> &tracked_objects_points, 
                                  std::unordered_map<int, int> &track_to_label_id_map, bool use_global_coordinate = true)
    {
        ...
        for(int i=0; i<ins_seg_result.size(); ++i)
        {
            if(ins_seg_result[i].label == "static")
            {
                ...
                for(int j=0; j<ins_seg_this->mask.rows; ++j)
                {
                    ...
                    {
                        int pixel_label = static_cast<int>(ins_seg_this->mask.at<uchar>(j,k));
                        if(pixel_label > 0) //exclude background
                        {
                            track_id_mask.at<uint16_t>(j, k) = label_to_instance_id_map_default[label_id_map_reversed[pixel_label]];
                        }
                    }
                }
        ...
        for(int i=0; i<ins_seg_result.size(); ++i)
        {
            if(ins_seg_result[i].label != "static")
            {
                track_to_label_id_map[ins_seg_result[i].track_id] = label_id_map_default[ins_seg_result[i].label];
        ...
        std::cout << "track_id_mask.size()=" << track_id_mask.size() << std::endl;

        // Define a mask image to visualize the invalid pixels
        cv::Mat invalid_mask = cv::Mat::zeros(depth_value_mat.rows, depth_value_mat.cols, CV_8UC1);
        ...
        for(int i=0; i<height; ++i) // Row
        {
            for(int j=0; j<width; ++j) // Col
            {
                ...
                float depth_value_free_point = depth_value - 0.2f; //CHG

                // Invalid depth value
                if (depth_value < depth_truncate_distance_min_ || depth_value > depth_truncate_distance_max_) {
                    point.is_valid = false;
                    cloud[i][j] = point;
                    invalid_mask.at<uchar>(i,j) = 255;
                    continue;
                }
                ...
                point_3d_free_point = camera_intrinsics_matrix_inverse_ * Eigen::Vector3d(j, i, 1) * depth_value_free_point;
                ...
                if(use_global_coordinate){
                    ...
                    point_3d_free_point = camera_to_global_matrix.block<3,3>(0,0) * point_3d_free_point + camera_to_global_matrix.block<3,1>(0,3);
                }
                ...
                if(instance_id > c_max_movable_object_instance_id)
                {
                    label_id = label_id_map_static[instance_id_to_label_map_default[instance_id]];
                }else{
                ...
                if(getFlagConsiderDepthNoise()){
                    point.sigma = 0.0012f + 0.0019f * (depth_value - 0.4f)*(depth_value - 0.4f); // Kinect V2 depth error model
                    if(point.sigma > 0.5f){
                        point.sigma = 0.5f;
                    }
                }else{
                ...
                static const float max_depth_for_guessed_points = (1 << C_VOXEL_NUM_AXIS_N) * C_VOXEL_SIZE * 0.5f; 
                if(depth_value < max_depth_for_guessed_points && instance_id <= c_max_movable_object_instance_id) // Consider only movable objects
                {
                ...
                point.color_h = 200;  //TODO: NOTE: This is a temporary solution.CHG
                point.is_valid = true;

                point.free_position.x() = point_3d_free_point.x(); //CHG
                point.free_position.y() = point_3d_free_point.y();
                point.free_position.z() = point_3d_free_point.z();

                cloud[i][j] = point;
            }
        }

        cv::imshow("invalid_mask", invalid_mask);
        
        std::cout << "tracked_objects_points.size()=" << tracked_objects_points.size() << std::endl;
        return 0;
    }
    ...
    float alignPointCloudToSource(const pcl::PointCloud<pcl::PointXYZ>::Ptr &source_cloud, const Eigen::Vector3f source_camera_position, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_to_align, pcl::PointCloud<pcl::PointXYZ>::Ptr &aligned_cloud)
    {
        ...
        // Save source_cloud and aligned_cloud to pcd files. Make source_cloud red and aligned_cloud green
        // static int count = 0;
        ...
        // {
        //     pcl::PointXYZRGB point;
        //     point.x = aligned_cloud->points[i].x;
        //     point.y = aligned_cloud->points[i].y;
        //     point.z = aligned_cloud->points[i].z;
        //     point.r = 0;
        //     point.g = 255;
        //     point.b = 0;
        //     cloud_rgb->points.push_back(point);
        // }
        // cloud_rgb->width = source_cloud->points.size() + aligned_cloud->points.size();
        // cloud_rgb->height = 1;

        // std::cout << "score=" << score << std::endl;
        // std::cout << "source_cloud->points.size()=" << source_cloud->points.size() << std::endl;
        // std::cout << "aligned_cloud->points.size()=" << aligned_cloud->points.size() << std::endl;

        // pcl::io::savePCDFileASCII("/home/clarence/ros_ws/semantic_dsp_ws/src/Semantic_DSP_Map/data/matching_result/" + std::to_string(count) + std::to_string(score) + ".pcd", *cloud_rgb);
        // count ++;
    ...
    float alignTemplatesToPointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr &source_cloud, const Eigen::Vector3f source_camera_position, int label_id, pcl::PointCloud<pcl::PointXYZ>::Ptr &aligned_cloud)
    {
        ...
        for(auto &template_cloud : template_clouds)
        {
            ...
            // float mhd = findBestTemplateTransformation(source_cloud, source_camera_position, template_cloud.makeShared(), aligned_cloud_temp);
            // if(mhd < smallest_mhd){
            //     smallest_mhd = mhd;
            //     aligned_cloud = aligned_cloud_temp;
            // }
            
            // float score = findBestTemplateTransformationPHD(source_cloud, source_camera_position, template_cloud.makeShared(), aligned_cloud_temp);
...
private:
    ...
    float findBestTemplateTransformationPHD(const pcl::PointCloud<pcl::PointXYZ>::Ptr &source_cloud, const Eigen::Vector3f &source_camera_position, const pcl::PointCloud<pcl::PointXYZ>::Ptr &template_cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr &aligned_cloud, bool filter_source_cloud = false)
    {
        ...
        std::cout << "source_cloud_filtered->points.size()=" << source_cloud_filtered->points.size() << std::endl;
        ...
        // std::cout << "Cloest point: " << closest_point << std::endl;
        // std::cout << "camera_position_to_closest_point_vec: " << camera_position_to_closest_point_vec << std::endl;
        ...
        // std::cout << "occupied_voxel_num: " << occupied_voxel_num << std::endl;
        ...
        // Make a point cloud with color to show source_cloud_filtered and transformed template cloud
        // pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_result(new pcl::PointCloud<pcl::PointXYZRGB>);
        // for(size_t i = 0; i < source_cloud_filtered->points.size(); ++i) {
        //     pcl::PointXYZRGB point;
        //     point.x = source_cloud_filtered->points[i].x;
        //     point.y = source_cloud_filtered->points[i].y;
        //     point.z = source_cloud_filtered->points[i].z;
        //     point.r = 255;
        //     point.g = 0;
        //     point.b = 0;
        //     cloud_result->points.push_back(point);
        // }
    ...
    float findBestTemplateTransformationPHDFSConsidered(const pcl::PointCloud<pcl::PointXYZ>::Ptr &source_cloud, const Eigen::Vector3f &source_camera_position, const pcl::PointCloud<pcl::PointXYZ>::Ptr &template_cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr &aligned_cloud, bool filter_source_cloud = false)
    {
        ...
        // Use the percentage of the intersection number to the occupied voxel number as the score
        // std::cout << "biggest_intersection_num=" << biggest_intersection_num << ", occupied_voxel_num=" << occupied_voxel_num << std::endl;
        return (float)biggest_intersection_num / occupied_voxel_num;
    }
