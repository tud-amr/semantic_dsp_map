 ...
 * 1. Note the position displacement between two update steps should be less than the map size. (Fixed by moving multiple steps)
...
public:
    ...
    void runSystemChecking(){
        if(C_VOXEL_NUM_AXIS_X_N + C_VOXEL_NUM_AXIS_Y_N + C_VOXEL_NUM_AXIS_Z_N + C_MAX_PARTICLE_NUM_PER_VOXEL_N > 31){
            std::cout << "ERROR: C_VOXEL_NUM_AXIS_X_N + C_VOXEL_NUM_AXIS_Y_N + C_VOXEL_NUM_AXIS_Z_N + C_MAX_PARTICLE_NUM_PER_VOXEL_N should be no more than 31" << std::endl;
            exit(-1);
        }
    ...
    void updateEgoCenterPos(const Eigen::Vector3f &pos){
        ...
        static const float max_moving_dist_once_x = (1 << (C_VOXEL_NUM_AXIS_X_N - 2)) * C_VOXEL_SIZE; // Half of the map size
        static const float max_moving_dist_once_y = (1 << (C_VOXEL_NUM_AXIS_Y_N - 2)) * C_VOXEL_SIZE; // Half of the map size
        static const float max_moving_dist_once_z = (1 << (C_VOXEL_NUM_AXIS_Z_N - 2)) * C_VOXEL_SIZE; // Half of the map size
        static const float max_moving_dist_once_all = std::min(std::min(max_moving_dist_once_x, max_moving_dist_once_y), max_moving_dist_once_z);

        Eigen::Vector3f moving_vector = pos - last_pos;
        Eigen::Vector3f moving_unit_vector = moving_vector.normalized();
        float moving_dist = moving_vector.norm();
        Eigen::Vector3f new_pos = last_pos;

        while(moving_dist > max_moving_dist_once_all){
            // Move by multiple steps
            new_pos = new_pos + moving_unit_vector * max_moving_dist_once_all;

            ego_center_pos = new_pos; // Update ego center position.
            updateRingbufferIndexParams(); // Update the ring buffer indexes

            moving_vector = pos - new_pos;
            moving_dist = moving_vector.norm();
        }
    ...
    inline void getParticlePosByIndex(const uint32_t &particle_index, Eigen::Vector3f &pos){
        pos[0] = PARTICLE_ARRAY[particle_index].pos.x;
        pos[1] = PARTICLE_ARRAY[particle_index].pos.y;
        pos[2] = PARTICLE_ARRAY[particle_index].pos.z;
    }
    ...
    /// @param pos
    /// @param weight
    inline void getParticlePosWeightByIndex(const uint32_t &particle_index, Eigen::Vector3f &pos, float &weight){
        pos[0] = PARTICLE_ARRAY[particle_index].pos.x;
        pos[1] = PARTICLE_ARRAY[particle_index].pos.y;
        pos[2] = PARTICLE_ARRAY[particle_index].pos.z;
        weight = PARTICLE_ARRAY[particle_index].pos.weight;
    }
    ...
    inline uint32_t addNewParticle(const Eigen::Vector3f &particle_pos){
        ...
        particle.time_stamp = global_time_stamp;
        particle.forget_count = 0;
        particle.pos.x = particle_pos[0];
        particle.pos.y = particle_pos[1];
        particle.pos.z = particle_pos[2];
        particle.pos.weight = C_PARTICLE_OCC_INIT_WEIGHT;
    ...
    inline void addNewParticleWithSemantics(const Eigen::Vector3f &particle_pos, const uint8_t &label_id, const uint16_t &track_id, uint32_t &voxel_index, uint32_t &particle_index){
        Particle particle;
        particle.status = Particle_Status::REGULAR_BORN;
        particle.time_stamp = global_time_stamp;
        particle.forget_count = 0;
    ...
    /// @param voxel_index Return the voxel index of the particle. 0xffffffff if failed.
    /// @param particle_index Return the particle index of the particle. 0xffffffff if failed.
    inline void addGuessedParticles(const Eigen::Vector3f &particle_pos, const uint8_t &label_id, const uint16_t &track_id, uint32_t &voxel_index, uint32_t &particle_index){
        Particle particle;
        particle.status = Particle_Status::GUESSED_BORN;
        particle.time_stamp = global_time_stamp;
        particle.forget_count = 0;
    ...
    inline void getParticlesAsPointCloud(const std::unordered_set<uint32_t> &ptc_indices, pcl::PointCloud<pcl::PointXYZ>::Ptr &ptc_cloud, double weight_threshold = 0.1){
        ...
        for(auto it = ptc_indices.begin(); it != ptc_indices.end(); ++it){
            if(PARTICLE_ARRAY[*it].pos.weight > weight_threshold){
                pcl::PointXYZ ptc;
    ...
    inline uint32_t moveParticleToNewPosition(const uint32_t &particle_index, const Eigen::Vector3f &new_pos){
        ...
        PARTICLE_ARRAY[particle_index].pos.x = new_pos[0];
        PARTICLE_ARRAY[particle_index].pos.y = new_pos[1];
        PARTICLE_ARRAY[particle_index].pos.z = new_pos[2];
    ...
    inline void moveParticlesInSetByTransformation(const std::unordered_set<uint32_t> &ptc_indices, const Eigen::Matrix4f &t_matrix, std::unordered_set<uint32_t> &new_ptc_indices)
    {
        ...
        for(auto it = ptc_indices.begin(); it != ptc_indices.end(); ++it){
            ...
            // noise << gaussian_random_calculator_.queryNormalRandomPresetSD(), gaussian_random_calculator_.queryNormalRandomPresetSD(), gaussian_random_calculator_.queryNormalRandomPresetSD();
            // new_particles[i].pos = t_matrix.block<3, 3>(0, 0) * PARTICLE_ARRAY[*it].pos + t_matrix.block<3, 1>(0, 3) + noise;

            Eigen::Vector3f pos_no_noise, old_pos;
            old_pos << PARTICLE_ARRAY[*it].pos.x, PARTICLE_ARRAY[*it].pos.y, PARTICLE_ARRAY[*it].pos.z;

            pos_no_noise = t_matrix.block<3, 3>(0, 0) * old_pos + t_matrix.block<3, 1>(0, 3);
            new_particles[i].pos.x = pos_no_noise.x() + gaussian_random_calculator_.queryNormalRandomPresetSD();
            new_particles[i].pos.y = pos_no_noise.y() + gaussian_random_calculator_.queryNormalRandomPresetSD();
            new_particles[i].pos.z = pos_no_noise.z() + gaussian_random_calculator_.queryNormalRandomPresetSD();
    ...
    inline void moveParticlesInSetsByTransformations(const std::vector<std::unordered_set<uint32_t>> &ptc_indices, const std::vector<Eigen::Matrix4f> &t_matrices, std::vector<std::unordered_set<uint32_t>> &new_ptc_indices)
    {
        ...
        for(int i=0; i<particle_vector_size; ++i){
            ...
            for(auto it = ptc_indices[i].begin(); it != ptc_indices[i].end(); ++it){
                ...
                pos_no_noise = t_matrices[i].block<3, 3>(0, 0) * old_pos + t_matrices[i].block<3, 1>(0, 3);

                new_particles[i][j].pos.x = pos_no_noise.x() + gaussian_random_calculator_.queryNormalRandomPresetSD();
                new_particles[i][j].pos.y = pos_no_noise.y() + gaussian_random_calculator_.queryNormalRandomPresetSD();
                new_particles[i][j].pos.z = pos_no_noise.z() + gaussian_random_calculator_.queryNormalRandomPresetSD();
    ...
    inline void calculateWeightSumInVoxel(const uint32_t &voxel_index, float &weight_sum)
    {
        ...
        for(uint32_t i=1; i < C_MAX_PARTICLE_NUM_PER_VOXEL; ++i) // The first particle (i=0) is the time particle. Skip.
        {
            if(!isParticleVacant(PARTICLE_ARRAY[particle_start_index+i], x_index, y_index, z_index)){
                weight_sum += PARTICLE_ARRAY[particle_start_index+i].pos.weight;
            }
        }
    }
    ...
    inline void calculateWeightAndSemanticsInVoxel(const uint32_t &voxel_index, float &weight_sum, float &guessed_weight, uint8_t &label_id, uint16_t &track_id)
    {
        ...
        for(uint32_t i=1; i < C_MAX_PARTICLE_NUM_PER_VOXEL; ++i) // The first particle (i=0) is the time particle. Skip.
        {
            if(!isParticleVacant(PARTICLE_ARRAY[particle_start_index+i], x_index, y_index, z_index)){
                ...
                if(PARTICLE_ARRAY[particle_start_index+i].pos.weight > 1.f){
                    PARTICLE_ARRAY[particle_start_index+i].pos.weight = 1.f;
                }

                if(PARTICLE_ARRAY[particle_start_index+i].status == Particle_Status::GUESSED_BORN){
                    guessed_weight += PARTICLE_ARRAY[particle_start_index+i].pos.weight;
                }else if(PARTICLE_ARRAY[particle_start_index+i].status == Particle_Status::UPDATED && PARTICLE_ARRAY[particle_start_index+i].pos.weight < C_PARTICLE_OCC_INIT_WEIGHT){
                ...
                track_id_weight_map[PARTICLE_ARRAY[particle_start_index+i].track_id] += PARTICLE_ARRAY[particle_start_index+i].pos.weight;
    ...
    /// @param label_id 
    /// @param track_id 
    inline void calculateWeightAndSemanticsInVoxelConsiderNeighbors(const uint32_t &voxel_index, const float &occ_threshold, float &weight_sum, float &guessed_weight, uint8_t &label_id, uint16_t &track_id)
    {
        uint32_t x_index, y_index, z_index; // Voxel index in the ring buffer
        voxelIdxToRingbufferIdx(voxel_index, x_index, y_index, z_index);
        ...
        // Get the neighbor voxel indices
        std::vector<uint32_t> neighbor_voxel_indices;
        std::vector<std::vector<uint32_t>> neighbor_voxel_xyz_indices;

        uint32_t x_index_small = x_index - 1;
        uint32_t y_index_small = y_index - 1;
        uint32_t x_index_big = x_index + 1;
        uint32_t y_index_big = y_index + 1;

        neighbor_voxel_indices.push_back(voxel_index);
        neighbor_voxel_xyz_indices.push_back({x_index, y_index, z_index});        
       
        if(x_index_small >= 1){
            uint32_t this_voxel_index = ringbufferXYZIdxToVoxelIdx(x_index_small, y_index, z_index);
            if(this_voxel_index < C_VOXEL_NUM_TOTAL && isVoxelValid(this_voxel_index, x_index_small, y_index, z_index)){
                neighbor_voxel_indices.push_back(this_voxel_index);
                neighbor_voxel_xyz_indices.push_back({x_index_small, y_index, z_index});
            }
        }

        if(x_index_big < C_VOXEL_NUM_AXIS_X-1){
            uint32_t this_voxel_index = ringbufferXYZIdxToVoxelIdx(x_index_big, y_index, z_index);
            if(this_voxel_index < C_VOXEL_NUM_TOTAL && isVoxelValid(this_voxel_index, x_index_big, y_index, z_index)){
                neighbor_voxel_indices.push_back(this_voxel_index);
                neighbor_voxel_xyz_indices.push_back({x_index_big, y_index, z_index});
            }
        }

        if(y_index_small >= 1){
            uint32_t this_voxel_index = ringbufferXYZIdxToVoxelIdx(x_index, y_index_small, z_index);
            if(this_voxel_index < C_VOXEL_NUM_TOTAL && isVoxelValid(this_voxel_index, x_index, y_index_small, z_index)){
                neighbor_voxel_indices.push_back(this_voxel_index);
                neighbor_voxel_xyz_indices.push_back({x_index, y_index_small, z_index});
            }
        }

        if(y_index_big < C_VOXEL_NUM_AXIS_Y-1){
            uint32_t this_voxel_index = ringbufferXYZIdxToVoxelIdx(x_index, y_index_big, z_index);
            if(this_voxel_index < C_VOXEL_NUM_TOTAL && isVoxelValid(this_voxel_index, x_index, y_index_big, z_index)){
                neighbor_voxel_indices.push_back(this_voxel_index);
                neighbor_voxel_xyz_indices.push_back({x_index, y_index_big, z_index});
            }
        }
        

        // Calculate the weight sum and track id in the voxel and its neighbors
        weight_sum = 0.f;
        guessed_weight = 0.f;
        std::vector<float> weight_sum_vec(neighbor_voxel_indices.size(), 0.f);

        for(int j=0; j<neighbor_voxel_indices.size(); ++j){
            uint32_t voxel_index_this = neighbor_voxel_indices[j];
            uint32_t particle_start_index =  voxel_index_this << C_MAX_PARTICLE_NUM_PER_VOXEL_N;

            for(uint32_t i=1; i < C_MAX_PARTICLE_NUM_PER_VOXEL; ++i) // The first particle (i=0) is the time particle. Skip.
            {
                if(!isParticleVacant(PARTICLE_ARRAY[particle_start_index+i], neighbor_voxel_xyz_indices[j][0], neighbor_voxel_xyz_indices[j][1], neighbor_voxel_xyz_indices[j][2])){
                    ...
                    if(PARTICLE_ARRAY[particle_start_index+i].status == Particle_Status::GUESSED_BORN){
                        ...
                        weight_sum_vec[j] += PARTICLE_ARRAY[particle_start_index+i].pos.weight;
                    }
        ...
        if(weight_sum_vec[0] > occ_threshold){
            // Check if the neighbor voxels are all empty, then the voxel is empty.
            bool all_empty = true;
            for(int j=1; j<weight_sum_vec.size(); ++j){
                if(weight_sum_vec[j] > occ_threshold){
                    all_empty = false;
                    break;
                }
            }
            if(all_empty){
                ...
                weight_sum = weight_sum_vec[0];
            }
        }else{
            // Check if more than two neighbor voxels are all big, then the voxel is big.
            int big_count = 0;
            for(int j=1; j<weight_sum_vec.size(); ++j){
                if(weight_sum_vec[j] > occ_threshold){
                    ++big_count;
            ...
            static const float infered_voxel_weight = occ_threshold + 0.1f;
            if(big_count >= 2){
                weight_sum = infered_voxel_weight;
            }else{              
                weight_sum = weight_sum_vec[0];
            }
        }
    ...
    /// @brief Determine if a voxel is occupied given its index and a weight threshold. Also return the track id, semantic id 
    ...
    void clear()
    {
        ...
        global_time_stamp = 0;
        for(int i=0; i<C_VOXEL_NUM_AXIS_X; ++i){
            voxel_time_stamps_x[i] = 0;
        }

        for(int i=0; i<C_VOXEL_NUM_AXIS_Y; ++i){
            voxel_time_stamps_y[i] = 0;
        }

        for(int i=0; i<C_VOXEL_NUM_AXIS_Z; ++i){
    ...
    void initialize(){
        ...
        map_p_max_const[0] = (C_VOXEL_NUM_AXIS_X >> 1) * C_VOXEL_SIZE;
        map_p_max_const[1] = (C_VOXEL_NUM_AXIS_Y >> 1) * C_VOXEL_SIZE;
        map_p_max_const[2] = (C_VOXEL_NUM_AXIS_Z >> 1) * C_VOXEL_SIZE;

        map_p_min_const[0] = -map_p_max_const[0];
        map_p_min_const[1] = -map_p_max_const[1];
        map_p_min_const[2] = -map_p_max_const[2];
        ...
        med_index_move_bit_const = low_index_move_bit_const + C_VOXEL_NUM_AXIS_Z_N;
        high_index_move_bit_const = med_index_move_bit_const + C_VOXEL_NUM_AXIS_Y_N;

        low_index_retrieve_mask_const = (1 << C_VOXEL_NUM_AXIS_Z_N) - 1;  // e.g. 0000 0000 0000 0000 0000 0000 1111 1111
        med_index_retrieve_mask_const = (1 << (C_VOXEL_NUM_AXIS_Y_N + C_VOXEL_NUM_AXIS_Z_N)) - 1;  // e.g. 0000 0000 0000 0000 1111 1111 1111 1111
        high_index_retrieve_mask_const = (1 << (C_VOXEL_NUM_AXIS_X_N + C_VOXEL_NUM_AXIS_Y_N + C_VOXEL_NUM_AXIS_Z_N)) - 1; // e.g. 1111 1111 1111 1111 1111 1111 1111 1111
#else
        // Row-major order. Useless if using morton code. 
        low_index_move_bit_const = 0;
        med_index_move_bit_const = low_index_move_bit_const + C_VOXEL_NUM_AXIS_X_N;
        high_index_move_bit_const = med_index_move_bit_const + C_VOXEL_NUM_AXIS_Y_N;

        low_index_retrieve_mask_const = (1 << C_VOXEL_NUM_AXIS_X_N) - 1;  // e.g. 0000 0000 0000 0000 0000 0000 1111 1111
        med_index_retrieve_mask_const = (1 << (C_VOXEL_NUM_AXIS_Y_N + C_VOXEL_NUM_AXIS_X_N)) - 1;  // e.g. 0000 0000 0000 0000 1111 1111 1111 1111
        high_index_retrieve_mask_const = (1 << (C_VOXEL_NUM_AXIS_Z_N + C_VOXEL_NUM_AXIS_Y_N + C_VOXEL_NUM_AXIS_X_N)) - 1; // e.g. 1111 1111 1111 1111 1111 1111 1111 1111
#endif
...
protected:
    ...
    inline void globalFramePostoVoxelIdx(const ParticleBasicState &pos, uint32_t &voxel_index)
    {
        Eigen::Vector3f map_frame_pos(pos.x - map_center_pos.x(), pos.y - map_center_pos.y(), pos.z - map_center_pos.z());
        uint32_t x_index, y_index, z_index;
        mapFramePostoVoxelIdx(map_frame_pos, voxel_index, x_index, y_index, z_index);
    }

    /// @brief  Calculate the voxel index from the position in the global frame
    inline void globalFramePostoVoxelIdx(const ParticleBasicState &pos, uint32_t &voxel_index, uint32_t &x_index, uint32_t &y_index, uint32_t &z_index)
    {
        Eigen::Vector3f map_frame_pos(pos.x - map_center_pos.x(), pos.y - map_center_pos.y(), pos.z - map_center_pos.z());
        mapFramePostoVoxelIdx(map_frame_pos, voxel_index, x_index, y_index, z_index);
    }
    ...
    inline void mapFramePostoVoxelIdx(const Eigen::Vector3f &pos, uint32_t &voxel_index, uint32_t &x_index, uint32_t &y_index, uint32_t &z_index)
    {
        ...
        uint32_t map_x_index = static_cast<uint32_t>((pos.x() - map_p_min_const[0]) * voxel_size_recip);
        uint32_t map_y_index = static_cast<uint32_t>((pos.y() - map_p_min_const[1]) * voxel_size_recip);
        uint32_t map_z_index = static_cast<uint32_t>((pos.z() - map_p_min_const[2]) * voxel_size_recip);
        
        // Check if the index is in the map space.
        if(map_x_index >= 0 && map_x_index < C_VOXEL_NUM_AXIS_X && map_y_index >= 0 && map_y_index < C_VOXEL_NUM_AXIS_Y && map_z_index >= 0 && map_z_index < C_VOXEL_NUM_AXIS_Z){
...
#if (STORAGE_TYPE == 1)
        // Column-major order
        voxel_index = (((x_index << C_VOXEL_NUM_AXIS_Y_N) | y_index) << C_VOXEL_NUM_AXIS_Z_N) | z_index;
#elif (STORAGE_TYPE == 2)
        // Morton code
        voxel_index = libmorton::morton3D_32_encode(x_index, y_index, z_index);
#else
        // Row-major order
        voxel_index = (((z_index << C_VOXEL_NUM_AXIS_Y_N) | y_index) << C_VOXEL_NUM_AXIS_X_N) | x_index;
#endif
    ...
    /// @param y_index Ring buffer index of y axis
    /// @param z_index Ring buffer index of z axis
    /// @return voxel_index Voxel index in storage
    inline uint32_t ringbufferXYZIdxToVoxelIdx(const uint32_t &x_index, const uint32_t &y_index, const uint32_t &z_index)
    {
#if (STORAGE_TYPE == 1)
        // Column-major order
        return (((x_index << C_VOXEL_NUM_AXIS_Y_N) | y_index) << C_VOXEL_NUM_AXIS_Z_N) | z_index;
#elif (STORAGE_TYPE == 2)
        // Morton code
        return libmorton::morton3D_32_encode(x_index, y_index, z_index);
#else
        // Row-major order
        return (((z_index << C_VOXEL_NUM_AXIS_Y_N) | y_index) << C_VOXEL_NUM_AXIS_X_N) | x_index;
#endif
    }
    ...
    inline void voxelIdxToRingbufferIdx(const uint32_t &voxel_index, uint32_t &x_index, uint32_t &y_index, uint32_t &z_index)
    {
        ...
        // Morton code
        uint_fast16_t x, y, z;
        libmorton::morton3D_32_decode(voxel_index, x, y, z);
        x_index = x;
        y_index = y;
        z_index = z;
#else
    ...
    {   
        ...
        pos.x() = map_x_index * C_VOXEL_SIZE + map_p_min_const[0];
        pos.y() = map_y_index * C_VOXEL_SIZE + map_p_min_const[1];
        pos.z() = map_z_index * C_VOXEL_SIZE + map_p_min_const[2];
    }

    /// @brief Calculate the position of a voxel in the global frame from the voxel index in the map frame
    inline void mapXYZIdxToGlobalPose(const uint32_t &map_x_index, const uint32_t &map_y_index, const uint32_t &map_z_index, ParticleBasicState &pos)
    {
        pos.x = map_x_index * C_VOXEL_SIZE + map_p_min_const[0] + map_center_pos.x();
        pos.y = map_y_index * C_VOXEL_SIZE + map_p_min_const[1] + map_center_pos.y();
        pos.z = map_z_index * C_VOXEL_SIZE + map_p_min_const[2] + map_center_pos.z();
    }
    ...
    inline void mapXYZIdxToRingbufferXYZIdx(const uint32_t &map_x_index, const uint32_t &map_y_index, const uint32_t &map_z_index, uint32_t &buffer_x_index, uint32_t &buffer_y_index, uint32_t &buffer_z_index)
    {
        ...
        oneAxisIdxCorrectionX(buffer_idx_x_temp, buffer_x_index);
        oneAxisIdxCorrectionY(buffer_idx_y_temp, buffer_y_index);
        oneAxisIdxCorrectionZ(buffer_idx_z_temp, buffer_z_index);
    }
    ...
    inline void ringbufferXYZIdxToMapXYZIdx(const uint32_t &buffer_x_index, const uint32_t &buffer_y_index, const uint32_t &buffer_z_index, uint32_t &map_x_index, uint32_t &map_y_index, uint32_t &map_z_index)
    {
        ...
        oneAxisIdxCorrectionX(map_idx_x_temp, map_x_index);
        oneAxisIdxCorrectionY(map_idx_y_temp, map_y_index);
        oneAxisIdxCorrectionZ(map_idx_z_temp, map_z_index);
    }


    /// @brief Correct the index of an axis if it is out of range for x axis
    inline void oneAxisIdxCorrectionX(const int &index_ori, uint32_t &index_corrected){
        // Correct the index of an axis if it is out of range
        if(index_ori < 0){
            index_corrected = index_ori + C_VOXEL_NUM_AXIS_X;
        }else if(index_ori >= C_VOXEL_NUM_AXIS_X){
            index_corrected = index_ori - C_VOXEL_NUM_AXIS_X;
        }else{
    ...
    /// @brief Correct the index of an axis if it is out of range for y axis
    inline void oneAxisIdxCorrectionY(const int &index_ori, uint32_t &index_corrected){
        // Correct the index of an axis if it is out of range
        if(index_ori < 0){
            index_corrected = index_ori + C_VOXEL_NUM_AXIS_Y;
        }else if(index_ori >= C_VOXEL_NUM_AXIS_Y){
            index_corrected = index_ori - C_VOXEL_NUM_AXIS_Y;
        }else{
            index_corrected = index_ori;
        }
    ...
    /// @brief Correct the index of an axis if it is out of range for z axis
    inline void oneAxisIdxCorrectionZ(const int &index_ori, uint32_t &index_corrected){
        // Correct the index of an axis if it is out of range
        if(index_ori < 0){
            index_corrected = index_ori + C_VOXEL_NUM_AXIS_Z;
        }else if(index_ori >= C_VOXEL_NUM_AXIS_Z){
            index_corrected = index_ori - C_VOXEL_NUM_AXIS_Z;
        }else{
            index_corrected = index_ori;
        }
    ...
    /// @brief Correct the index of an axis if it is out of range for x axis
    inline void oneAxisIdxCorrectionX(int &index){
        // Correct the index of an axis if it is out of range
        if(index < 0){
            index = index + C_VOXEL_NUM_AXIS_X;
        }else if(index >= C_VOXEL_NUM_AXIS_X){
            index = index - C_VOXEL_NUM_AXIS_X;
        }else{
            // Do nothing
        }
    ...
    /// @brief Correct the index of an axis if it is out of range for y axis
    inline void oneAxisIdxCorrectionY(int &index){
        // Correct the index of an axis if it is out of range
        if(index < 0){
            index = index + C_VOXEL_NUM_AXIS_Y;
        }else if(index >= C_VOXEL_NUM_AXIS_Y){
            index = index - C_VOXEL_NUM_AXIS_Y;
        }else{
            // Do nothing
        }
    ...
    /// @brief Correct the index of an axis if it is out of range for z axis
    inline void oneAxisIdxCorrectionZ(int &index){
        // Correct the index of an axis if it is out of range
        if(index < 0){
            index = index + C_VOXEL_NUM_AXIS_Z;
        }else if(index >= C_VOXEL_NUM_AXIS_Z){
            index = index - C_VOXEL_NUM_AXIS_Z;
        }else{
            // Do nothing
        }
    ...
    void updateRingbufferIndexParams()
    {
        ...
        if(new_moved_index_x > 0){
            for(int i=0; i<new_moved_index_x; ++i){
                ...
                oneAxisIdxCorrectionX(buffer_idx_x_temp);
                voxel_time_stamps_x[buffer_idx_x_temp] = global_time_stamp;
            }
        }else if (new_moved_index_x < 0)
        {
            for(int i=0; i<-new_moved_index_x; ++i){
                int buffer_idx_x_temp = C_VOXEL_NUM_AXIS_X - 1 - i + buffer_moved_equivalent_steps_x;
                oneAxisIdxCorrectionX(buffer_idx_x_temp);
                voxel_time_stamps_x[buffer_idx_x_temp] = global_time_stamp;
            }
        }else{
        ...
        if(new_moved_index_y > 0){
            for(int i=0; i<new_moved_index_y; ++i){
                ...
                oneAxisIdxCorrectionY(buffer_idx_y_temp);
                voxel_time_stamps_y[buffer_idx_y_temp] = global_time_stamp;
            }
        }else if (new_moved_index_y < 0)
        {
            for(int i=0; i<-new_moved_index_y; ++i){
                int buffer_idx_y_temp = C_VOXEL_NUM_AXIS_Y - 1 - i + buffer_moved_equivalent_steps_y;
                oneAxisIdxCorrectionY(buffer_idx_y_temp);
                voxel_time_stamps_y[buffer_idx_y_temp] = global_time_stamp;
            }
        }else{
        ...
        if(new_moved_index_z > 0){
            for(int i=0; i<new_moved_index_z; ++i){
                ...
                oneAxisIdxCorrectionZ(buffer_idx_z_temp);
                voxel_time_stamps_z[buffer_idx_z_temp] = global_time_stamp;
            }
        }else if (new_moved_index_z < 0)
        {
            for(int i=0; i<-new_moved_index_z; ++i){
                int buffer_idx_z_temp = C_VOXEL_NUM_AXIS_Z - 1 - i + buffer_moved_equivalent_steps_z;
                oneAxisIdxCorrectionZ(buffer_idx_z_temp);
                voxel_time_stamps_z[buffer_idx_z_temp] = global_time_stamp;
            }
        }else{
        ...
        getEquivalentStepsX(buffer_moved_steps_x, buffer_moved_equivalent_steps_x);
        getEquivalentStepsY(buffer_moved_steps_y, buffer_moved_equivalent_steps_y);
        getEquivalentStepsZ(buffer_moved_steps_z, buffer_moved_equivalent_steps_z);
    }

    /// @brief Calculate the equivalent steps of a given steps considering ringbuffer. For x axis.
    /// @param ori_steps 
    /// @param equivalent_steps 
    inline void getEquivalentStepsX(const int &ori_steps, int &equivalent_steps){
        if(ori_steps > 0){
            equivalent_steps = ori_steps % C_VOXEL_NUM_AXIS_X;
        }else if(ori_steps < 0){
            equivalent_steps = -(-ori_steps % C_VOXEL_NUM_AXIS_X);
        }else{
    ...
    /// @brief Calculate the equivalent steps of a given steps considering ringbuffer. For y axis.
    /// @param ori_steps
    /// @param equivalent_steps
    inline void getEquivalentStepsY(const int &ori_steps, int &equivalent_steps){
        if(ori_steps > 0){
            equivalent_steps = ori_steps % C_VOXEL_NUM_AXIS_Y;
        }else if(ori_steps < 0){
            equivalent_steps = -(-ori_steps % C_VOXEL_NUM_AXIS_Y);
        }else{
            equivalent_steps = ori_steps; //0
    ...
    /// @brief Calculate the equivalent steps of a given steps considering ringbuffer. For z axis.
    /// @param ori_steps
    /// @param equivalent_steps
    inline void getEquivalentStepsZ(const int &ori_steps, int &equivalent_steps){
        if(ori_steps > 0){
            equivalent_steps = ori_steps % C_VOXEL_NUM_AXIS_Z;
        }else if(ori_steps < 0){
            equivalent_steps = -(-ori_steps % C_VOXEL_NUM_AXIS_Z);
        }else{
            equivalent_steps = ori_steps; //0
    ...
    inline bool calculateParticleBasicStateInImage(const Particle &ptc, const Eigen::Matrix4f& extrinsic_matrix, const Eigen::Matrix3f& intrinsic_matrix, int &row, int &col, float &camera_frame_z)
    {
        // Convert point to homogeneous coordinates
...
private:
                        ...
                        for (int dz = -1; dz <= 0; dz++) {
                            ...
                            for(int i=1; i<C_MAX_PARTICLE_NUM_PER_VOXEL; ++i){
                                ...
                                if(ptc->status != Particle_Status::INVALID){
                                    ...
                                    if(calculateParticleBasicStateInImage(*ptc, extrinsic_matrix, intrinsic_matrix, row, col, camera_frame_z)){
                                        ...
                                        if(depth_img.at<float>(row, col) > g_depth_range_max){
                                            ...
                                            // ptc->status = Particle_Status::INVALID;
                                            ptc->pos.weight = C_PARTICLE_OCC_INIT_WEIGHT;
                                        ...
                                        // The particle will be stored in particle_to_pixel_index_map.
                            ...
                            if(voxel_observed){
                                ...
                                if(valid_particle_num_in_voxel == 0){
                                    ...
                                    if(calculateParticleBasicStateInImage(imaginary_particle, extrinsic_matrix, intrinsic_matrix, row, col, camera_frame_z)){
    ...
    private:
        GaussianRandomCalculator gaussian_random_calculator_;
};
